...
use std::{
    ffi::{c_int, c_long, c_char, CStr, CString},
...
impl C2paStream {
    pub fn new(context: *mut StreamContext, read: ReadCallback, seek: SeekCallback, write: WriteCallback) -> Self {
        Self {
...
impl Read for C2paStream {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        let bytes_read = unsafe { (self.read)(context, buf.as_mut_ptr(), buf.len() as usize) };
        if bytes_read < 0 {
...
/// 
#[no_mangle]
pub unsafe extern "C" fn c2pa_manifest_reader_read(reader_ptr:  *mut *mut ManifestStoreReader, format: *const c_char, stream: C2paStream) -> *mut c_char {
...
///
#[no_mangle]
pub unsafe extern "C" fn c2pa_manifest_reader_resource(reader_ptr:  *mut *mut ManifestStoreReader, manifest_label: *const c_char, id: *const c_char, stream: C2paStream) -> () {
    let reader = Box::from_raw(*reader_ptr);
